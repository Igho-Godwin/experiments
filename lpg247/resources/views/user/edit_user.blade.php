                        ...
                        <div class='display-inline-block width-50' >
                           ...
                           <form id='editUserForm'>
                                ...
                                <div class='text-center'>
                                    <img  id='picture' class='picture rounded-circle' src='@if(Auth::user()->picture == null)images/default-pic.png @else {{$marketer_obj->fitImage(Auth::user()->picture,200,200)}}  @endif' style='max-width:200px;max-height:200px;' />
                                ...
                                <div class='col-sm-12'>
                                    <div class='row'>
                                        <div class='col-sm-6 margin-top-20'>
                                            <input type='text' name='fullName'     class='form-control' required=''     placeholder='Full Name' value='{{Auth::user()->fullName}}' />
                                        </div>
                                
                                        <div class='col-sm-6 margin-top-20'>
                                            <input type='text' name='email'         class='form-control' required=''        placeholder='Email Address' value='{{Auth::user()->email}}' />
                                        </div>
                                ...
                                <div class='col-sm-12'>
                                    ...
                                    <div class='row'>
                                        <div class='col-sm-6 margin-top-20'>
                                            <input type='text' name='phoneNumber'         class='form-control' value='{{Auth::user()->phoneNumber}}' required=''        placeholder='Phone Number' />
                                        </div>
                                
                                        <div class='col-sm-6 margin-top-20'>
                                            <select id='user_category' name='user_category' class="form-control" >
                                            <option value=''>Select</option>
                                            <option value='0' @if(Auth::user()->user_category == 0) Selected @endif>Consumer</option>
                                            <option value='1' @if(Auth::user()->user_category == 1) Selected @endif>Marketer</option>
                                        </select>
                                        </div>
                                    </div>
                                ...
                                <div class='col-sm-12 @if(Auth::user()->user_category !== 1) hide @endif' >
                                    ...
                                    <div class='margin-top-20'>
                                        <select id='marketer_type' name='marketer_type' class="form-control" title='Marketer Type' >
                                            ...
                                            <option value='0' @if(Auth::user()->marketer_type == 0) Selected @endif>Retail</option>
                                            <option value='1' @if(Auth::user()->marketer_type == 1) Selected @endif>Whole Sale</option>
                                        </select>
                                    </div>
                                </div>
                                @if(Auth::user()->major_marketer != null)
                                    <?php $major_marketer = explode(',',Auth::user()->major_marketer); ?>
                                @endif
                                
                                <div class="col-sm-12 @if(Auth::user()->marketer_type !== 1) hide @endif" id='major_marketers'>
                                        <div class='margin-top-20'>
                                           <select name='major_marketer[]' class="form-control form-control-lg " title='Marketer Type' onChange='chooseMajorMarketers()' multiple>
                                            ...
                                            @foreach($MajorMarketers as $major_maketer_values)
                                                <option @if(in_array($major_maketer_values->id,$major_marketer)) Selected @endif value='{{$major_maketer_values->id}}' >{{$major_maketer_values->marketer_name}}</option>
                                            @endforeach
                                    </select>
                                        </div>
                                    </div>
                                ...
                                <div class='col-sm-12 @if(Auth::user()->user_category !== 1) hide @endif' >
                                    ...
                                    <div class='row'>
                                        <div class='col-sm-6 margin-top-20'>
                                            ...
                                            <option value='1' @if(Auth::user()->do_delivery == 1) Selected @endif>Yes</option>
                                            <option value='2' @if(Auth::user()->do_delivery == 2) Selected @endif>No</option>
                                            </select>
                                        
                                        </div>
                                
                                        <div class='col-sm-6 margin-top-20'>
                                            <input type='text' name                ='cost_of_delivery' title='cost of delivery'  class='form-control' value='{{Auth::user()->cost_of_delivery}}'  required='' placeholder    ='Cost of Delivery Per Km' />
                                        </div>
                                ...
                                <div class='col-sm-12'>
                                    <div class='margin-top-20'>
                                        <input type='text' name='address' id='autocomplete' class='form-control' value='{{Auth::user()->address}}' required='' placeholder='Address' />
                                    </div>
                                </div>
                                
                                <div id='documents' class="col-sm-12 @if(Auth::user()->user_category !== 1) hide @endif">
                                    <div class="margin-top-20">
                                        <textarea cols='45' name='about_company' class="form-control" rows='5'  placeholder='About Company'>{{Auth::user()->about_company}}</textarea>
                                    </div>
                                </div>
