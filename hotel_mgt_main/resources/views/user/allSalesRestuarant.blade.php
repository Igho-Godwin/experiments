                    ...
                    <div class="col-sm-12">
                         <br>
                          <div style='width:50%;margin-left:auto;margin-right:auto;'>
                            ...
                            <form id='search-form' action='allSalesRestuarant' method='get' >
                                ...
                                <div class="table-responsive">
                                    ...
                                    <tbody>
                                        @foreach($all_items as $data)
                                           <tr>
                                              <td>{{$food_obj->getFoodName($data->item)}}</td>
                                              <td>&#8358;{{number_format($data->price)}}</td>
                                              <td>{{$data->qty}}</td>
                                              <td>{{$food_obj->getModeOfPayment($data->mode_of_payment)}}</td>
                                              <td>{{$user->getName($data->added_by)}}</td>
                                              <td>{{date('d-m-Y H:i:s',strtotime($data->updated_at))}}</td>
                                              <td class='ed'>
                                                <a href='editSalesRestuarant?id={{$data->id}}'>Edit</a>   
                                              </td>
                                              <td class='ed'>
                                                  <a href='#' onClick='deleteSalesRestuarant({{$data->id}})'>Delete</a> 
                                              </td>
                                           </tr>
                                        @endforeach
                                    </tbody>
