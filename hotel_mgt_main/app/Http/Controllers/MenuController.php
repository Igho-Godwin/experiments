   ...
   // public function getDeptIncome($)
    ...
    public function BestPerformingDepts()
    {
        ...
        if(Request::get('date1') == null)
        {
               ...
               $expenses_room = DB::table('store')->select(DB::raw('itemName, sum(qty * unitPrice) as total_expenses, max(id) as max_id '))
                                                   ->where('status','0')
                                                   ->where('dept','2')
                                                   ->whereRaw("Year(created_at) = '".date('Y')."' and month(created_at) = '".date('m')."'   ")
                                                   ->groupBy('itemName')
               ...
               $expenses_drink = DB::table('store')->select(DB::raw('itemName, sum(qty * unitPrice) as total_expenses, max(id) as max_id '))
                                                   ->where('status','0')
                                                   ->where('dept','4')
                                                   ->whereRaw("Year(created_at) = '".date('Y')."' and month(created_at) = '".date('m')."'   ")
                                                   ->groupBy('itemName')
               ...
               $expenses_restuarant = DB::table('store')->select(DB::raw('itemName, sum(qty * unitPrice) as total_expenses, max(id) as max_id '))
                                                   ->where('status','0')
                                                   ->where('dept','3')
                                                   ->whereRaw("Year(created_at) = '".date('Y')."' and month(created_at) = '".date('m')."'   ")
                                                   ->groupBy('itemName')
               ...
               $expenses_pool = DB::table('store')->select(DB::raw('itemName, sum(qty * unitPrice) as total_expenses, max(id) as max_id '))
                                                   ->where('status','0')
                                                   ->where('dept','5')
                                                   ->whereRaw("Year(created_at) = '".date('Y')."' and month(created_at) = '".date('m')."'   ")
                                                   ->groupBy('itemName')
      ...
      //  $this->clearCart();
       ...
       // dd(Session::get('qty'));
        ...
        {  
          ...
          return view('user.shopping-cart')->with(['menu_obj'=>$menu_obj,'rooms'=>$rooms,'obj'=>$this]);  
    ...
    public function RoomDetails()
    {
        ...
        if(Auth::check())
        {
            ...
            if(Request::get('a')== null){
              ...
              Session::put('token',$success['token']);
              $countries = countries::all();
              return view('user.roomDetails')->with(['room_details'=>$room_details,'','d'=>$d,'countries'=>$countries,'customer_details'=>$customer_details]);  
        }   
        else{
            return redirect('login');
        }
