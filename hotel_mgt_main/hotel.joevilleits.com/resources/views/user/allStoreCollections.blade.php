                                    ...
                                    <tbody>
                                        @foreach($all_items as $data)
                                           <tr>
                                              ...
                                              <td>{{$data->qty}}</td>
                                              <td>&#8358;{{number_format($data->qty * $data->unit_price)}}</td>
                                              <td>{{$user->getName($data->user_id)}}</td>
                                              <td>{{$user->getName($data->added_by)}}</td>
                                              <td>{{date('d-m-Y H:i:s',strtotime($data->updated_at))}}</td>
                                              <td class='ed'>
                                                <a href='editStoreCollections?id={{$data->id}}'>Edit</a>   
                                              </td>
                                              <td class='ed'>
                                                  <a href='#' onClick='deleteStoreCollections({{$data->id}})'>Delete</a> 
                                              </td>
                                           </tr>
                                        @endforeach
                                    </tbody>
